...
const createToken = (user, time) => {
	return jwt.sign({identification: user.identification, fullName: user.fullName}, config.SECRET, {
		expiresIn: time
	})
}
...
export const changePassword = async (req, res) => {
	...
	try {
		...
		const user = await User.findOneAndUpdate({identification: identification}, {password: newPassword}, {new: true})
		if(user){
